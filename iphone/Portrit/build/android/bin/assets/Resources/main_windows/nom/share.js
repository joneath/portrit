...
function submit_nom(e){
    ...
    if (new_photo){
        ...
        check_upload_interval = setInterval(function(){
            ...
            if (check_count > 25 && (!progress || progress <= 5)){
                clearInterval(check_upload_interval);
                win.animate(fadeOut);
                Ti.App.fireEvent('close_settings_page', { });
                Ti.App.fireEvent('update_my_photos', { });
            ...
            else if (upload_ready_state && parseInt(upload_ready_state) < 4 && check_count > 15 && (!progress || progress <= 5)){
                check_count = 0;
                Ti.App.fireEvent('restart_upload', { });
            }
            else if (check_count > 25 && progress > 0 && (progress - prev_progress == 0)){
                if (timeout_count >= 5){
                    alert('Upload timeout. Restarting...');
                    check_count = 0;
                    timeout_count = 0;
                    Ti.App.fireEvent('restart_upload', { });
                }
