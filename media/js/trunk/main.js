$(document).ready(function(){
    ...
    function render_settings(){
        ...
        if (account_type == 'BASIC'){
            style = 'style="color: #69CCCB;"';
        }
        else if(account_type == 'SOCIAL'){
            style = 'style="color: #99CB6E;"';
        }
        else if(account_type == 'PLUS'){
            style = 'style="color: #CB6698;"';
        }
    ...
    function set_mobile_css(){
        ...
        if (mobile && !tablet){
            ...
            if (typeof(_gaq) !== "undefined"){
                meta_html = '<link rel="stylesheet" href="/site_media/styles/production/tablet.css"/>' +
                                '<link rel="shortcut icon" href="/site_media/img/favicon.ico">' +
                                '<link rel="apple-touch-icon" href="/site_media/img/icon128.png"/>' +
                                '<link rel="apple-touch-icon-precomposed" href="/site_media/img/icon128.png"/>';
            }
            else{
                meta_html = '<link rel="stylesheet" href="/site_media/styles/trunk/tablet.css"/>' +
                                '<link rel="shortcut icon" href="/site_media/img/favicon.ico">' +
                                '<link rel="apple-touch-icon" href="/site_media/img/icon128.png"/>' +
                                '<link rel="apple-touch-icon-precomposed" href="/site_media/img/icon128.png"/>';
            }
        ...
        if (method == 'new_nom'){
            ...
            method_html =   '<div class="nom_cat_' + data.nomination_category.replace(' ', '_').toLowerCase() + ' notification_color"></div>' +
                            '<div class="notification_text_cont">' +
                                '<p class="strong">' + nominator + '</p>' + '<span> nominated your photo for the </span><p class="strong">' + data.nomination_category + '</p><span> trohpy!</span>' +
                            '</div>';
            ...
            if (data.nominatee == me.id){
                ...
                method_html =   '<div class="nom_cat_' + data.nomination_category.replace(' ', '_').toLowerCase() + ' notification_color"></div>' +
                                '<div class="notification_text_cont">' +
                                    '<p class="strong">' + data.nominatee_name + '\'s</p>' + '<span> photo won the <p class="strong">' + data.nomination_category + '</p> nomination!</span>' +
                                '</div>';
            }
        }
        
        notification_html = '<div class="notification_popup" style="display:none;" value="' + data.id + '">' +
                                '<img class="close_notification" class="close_img" src="/site_media/img/x.png">' +
    ...
    function update_notifications(data, method){
        ...
        if (method == 'new_nom'){
            ...
            notification_html = '<div class="notification_popup_cont color_transition nom_cat_'  + nom_cat + '" name="' + nom_cat + '" value="' + data.id + '" read="0" id="notification_' + data.notification_id + '">' +
            ...
            notification_html = '<div class="notification_popup_cont color_transition nom_cat_'  + nom_cat + '" name="' + nom_cat + '" value="' + data.id + '" read="0" won="true" id="notification_' + data.notification_id + '">' +
                                    '<div class="nom_cat_' + nom_cat +' notification_color"></div>' +
                                    '<div class="notification_text_cont">' +
                                        '<p class="strong" style="">' + name +'</p>' +
        ...
        $('#footer_notification_cont > img').css({
           'opacity': '1.0' 
        });
    ...
    function render_notifications(data){
        ...
        for (var i = 0; i < data.length; i++){
            ...
            if (data[i].notification_type == 'new_nom'){
                notification_html +='<div class="notification_popup_cont ' + color_class + '" name="' + nom_cat + '" value="' + data[i].nomination + '" read="' + data[i].read + '" id="notification_' + data[i].notification_id + '" time="' + data[i].create_time +'">' +
                                        '<div class="nom_cat_' + nom_cat + ' notification_color"></div>' +
                                        '<div class="notification_text_cont">' +
                                        ...
                                        '</div>' +
                                        '<img class="kill_notification" value="' + data[i].notification_id + '" src="/site_media/img/x.png" style="display:none;"/>' +
                                    '</div>'
            }
                ...
                notification_html +='<div class="notification_popup_cont ' + color_class + '" name="' + nom_cat + '" value="' + data[i].nomination + '" read="' + data[i].read + '" id="notification_' + data[i].notification_id + '" won="true" time="' + data[i].create_time +'">' +
                                        '<div class="nom_cat_' + nom_cat + ' notification_color"></div>' +
                                        '<div class="notification_text_cont">' +
    ...
    $('.notification_popup_cont').live('mouseover mouseout', function(event) {
        ...
        if (event.type == 'mouseover') {
            $(this).addClass('nom_cat_' + cat_name);
            $(this).css({
                'color': 'white'
            });
            $(this).find('p').css({
    ...
    function init_view(func_ptr){
        $('#cont').append('<img id="login_loader" src="/site_media/img/loader_main.gif"/>');
        if (window.localStorage !== undefined){
        ...
        for (var i = 0; i < friend_array.length; i++){
            if (friend_array[i].hidden === false){
                ...
                if (first_alpha){
                    ...
                    name_html = '<div class="text_wrapper"><div class="text" onclick="void(0)"><h3>' + friend_array[i].name + '</h3></div><div class="friend_controls"></div></div>';
    ...
    $('#recent_noms').live('click', function(){
        ...
        if (default_view === 'friend'){
            ...
            if ($('#wall_cont #profile_wrap').length == 0){
                ...
                var wall_html = '<div id="profile_wrap">' +
                                    '<div id="activate_latest_photos_shadow"></div>' +
            ...
            $('#wall_cont').hide();
            var wall_html = '<div id="user_profile_cont"><div id="profile_nav_cont" value="albums"><h2>Photo Albums</h2><img id="album_left_arrow" style="display:none;" src="/site_media/img/albumarrow_left.png"/><img id="album_right_arrow" src="/site_media/img/albumarrow.png"/></div><div id="user_profile"></div><div id="album_cont" style="display:none;"></div></div>';
            $('#wall_cont').append(wall_html);
            selected_user = 'me';
    ...
    function render_user_trophies(data){
        ...
        for (var i = 0; i < data.length; i++){
            ...
            trophy_html =   '<div class="trophy_cont nom_cat_' + trophy_name_underscore + '_hover">' +
                                '<a href="#/trophy/me/' + trophy_name_underscore + '">' +
    ...
    function nom_detail_view(){
        ...
        if (won != undefined){
            ...
            title = '<h1 class="title" name="' + nom.nomination_category + '">Winning <span class="nom_cat_' + nom.nomination_category.replace(' ', '_').toLowerCase() + '_text">' + nom.nomination_category + '</span> Photos</h1>';
        }
        else{
                                    ...
                                    '<div id="main_nom_cont_right_wrap">' +
                                        ...
                                        '<div id="vote_cont">' +
                                            '<div id="vote_cont_left">' +
                                                '<img class="vote vote_up" name="' + nom.id + '" src="/site_media/img/up_detailed.png"/>' +
                                                '<img class="vote vote_down" name="' + nom.id + '" src="/site_media/img/down_detailed.png"/>' +
                                                '<p>' + nom.vote_count + '</p>' +
                                            '</div>' +
                                            '<div class="clear"></div>' +
                                        '</div>' +
        ...
        var html = '<div id="nom_detail_cont">' + title + nom_stream_html + nom_main_cont_html + nom_bottom_cont_html + '</div>';
    ...
    function get_nom_comments(id, won){
        ...
        if (active_cache[id].comments === false){
    ...
    function render_winning_photo_noms(data){
        for (var i = 0; i < data.album_objs.length; i++){
            $('#' + data.album_objs[i].fid).append('');
        }
    }
    ...
    function main_photo_view(){
        ...
        if (friends[id] != undefined){
            album_html = '<div id="info_wrap" style="display:none;"><h1>Profile Info</h1><div id="info_cont"><div id="info_loading"><img src="/site_media/img/album-loader-dark.gif"/></div></div></div><div id="album_cont"></div><div id="profile_wrap" style="display:none"><h1>' + friends[id].name.split(" ", 1) + '\'s Stream</h1><a id="add_new_comment" class="awesome large">Post</a><div id="new_comment_cont"><div class="comment_top_head"><a id="cancel_new_comment" class="awesome large">Close</a><a id="post_new_comment" class="awesome large">Post</a></div><textarea id="new_comment_body" class="comment_body"></textarea></div><div id="profile_cont_wrap"><div id="profile_loading"><img src="/site_media/img/ajax-loader-light.gif"/></div><div id="scroller"><div id="profile_cont"></div></div></div></div>';
            if (!mobile || tablet){
                $('#title').html('<h3>' + name + '</h3><div id="main_view_control"><a id="user_info_view" class="main_control awesome large">Info</a><a id="user_photo_view" class="main_control awesome large main_control_active">Photos</a><a id="user_wall_view" class="main_control awesome large">Stream</a></div>');
            }
            else if (mobile && !tablet){
                $('#title').html('<h3>' + name + '</h3><div id="main_view_control"><a id="user_info_view" class="main_control awesome large">Info</a><a id="user_photo_view" class="main_control awesome large main_control_active">Photos</a><a id="user_wall_view" class="main_control awesome large">Stream</a></div>');
            }
                    ...
                    if (k == 0){
                        ...
                        nom_heading_html =  '<div class="nom_cat_top_cont nom_cat_' + nom_cat_underscore + '">' +
                                                ...
                                                '<div class="nom_cat_text_cont">' +
                                                    '<a href="#/trophy/cat=' +  nom_cat_underscore + '">' +
                        ...
                        photo_html ='<div class="stream_photo_cont">' +
                                        '<div class="nom_cat_stream_cont">' + 
                                            '<div id="nom_photo_' + nom_data[k].id + '" class="nom_photo_thumbnail" name="selected" style="opacity: 1.0; left:' + small_photo_offset + 'px;">' +
                                                ...
                                                '<p class="nom_cat_' + nom_data[k].nomination_category.replace(' ', '_').toLowerCase() + '">' + nom_data[k].vote_count + '</p>' +
                                            '</div>' +
                                        '</div>' +
                                        '<div class="nom_cat_main_photo_cont">' +
                                            ...
                                            '<div class="nom_cat_main_photo_wrap">' +
                                                ...
                                                '</a>' +
                                                '<div class="vote_cont">' +
                                                    '<img class="vote_up" name="' + nom_data[k].id + '" src="/site_media/img/up_small.png"/>' +
                                                    '<p>' + nom_data[k].vote_count + '</p>' +
                                                    '<img class="vote_down" name="' + nom_data[k].id + '" src="/site_media/img/down_small.png"/>' +
                                                '</div>' +
                                            '</div>' +
                                        '</div>' +
    ...
    function render_profile(data){
        ...
        if (selected_user !== 'me'){
            // mutual_friend_data = null;
            // mutual_friend_top = 0;
            // get_mutual_friends(data.id);
            
            $('#info_wrap').append('<h1>Trophy Room</h1><div id="trophy_cont"></div>');
            
            render_user_trophy_room(getUrlVars().user);
        }
    ...
    function render_user_trophy_room(uid){
        
    }
    
    // function render_user_likes(data){
    //     $('#info_wrap ul').append();
    //     user_likes = data;
    // }
    // 
    // function render_user_movies(data){
    //     $('#info_wrap ul').append();
    //     user_movies = data;
    // }
    // 
    // function render_user_books(data){
    //     $('#info_wrap ul').append();
    //     user_books = data;
    // }
    ...
    function get_user_feed(fnc_ptr, method){
        if (method == 'user_stream'){
            ...
            $.getJSON('/get_recent_winners/', function(data){
                ...
                for (var i = 0; i < data.length; i++){
                    nom_data = data[i].noms;
                    winning_nom_cats_map[data[i].cat_name.replace(' ', '_').toLowerCase()] = [ ];

                    if (nom_data.length > 0){
                        for (var k = 0; k < nom_data.length; k++){
                            winning_noms_cache[nom_data[k].id] = nom_data[k];
                            winning_nom_cats_map[nom_data[k].nomination_category.replace(' ', '_').toLowerCase()].push(nom_data[k].id);
                        }
                    }
                }
                if (fnc_ptr){
        ...
        if (data){
            ...
            for (var i = 0; i < data.nom_data.length; i++){
                ...
                if ($(container).hasClass('empty')){
                    ...
                    photo_stream_html = '<img class="nom_photo_thumbnail" id="nom_photo_' + nom_data.id + '" name="selected" style="opacity: 1.0; left:' + small_photo_offset + 'px" src="' + nom_data.photo.src_small + '"/>';
                    photo_html ='<a href="">' +
                    ...
                    photo_stream_html = '<img class="nom_photo_thumbnail" id="nom_photo_' + nom_data.id + '" style="opacity: 0.6; left:' + small_photo_offset + 'px" src="' + nom_data.photo.src_small + '"/>';
    ...
    function init_recent_view(){
        ...
        // }
        // else{
        //     render_recent_stream(recent_stream_map);
        // }
    }
    ...
    function init_info_view(){
        ...
        if (user_profile === null){
            ...
            //     render_user_likes(response);
            // });
            // 
            // FB.api('/' + id + '/movies', function(response){
            //     render_user_movies(response);
            // });
            // 
            // FB.api('/' + id + '/books', function(response){
            //     render_user_books(response);
            // });
        }
        else{
            if ($('#info_wrap').children().length === 1){
    ...
    // function prev_album(e){
    //     for (var x = 0; x < albums.length; x++){
    //         if (albums[x].album.id === selected_album_id){
    //             if (x - 1 < 0){
    //                 x = 0;
    //             }
    //             else{
    ...
    //             }
    //             selected_photo = '';
    //             selected_album_id = albums[x].album.id;
    ...
    // }
    // 
    // function next_album(e){
    //     for (var x = 0; x < albums.length; x++){
    //         if (albums[x].album.id === selected_album_id){
    //             if (x + 1 > albums.length){
    //                 x = 0;
    //             }
    //             else{
    //                 x += 1;
                    ...
                    if ($('#back_gallery').length <= 0){
                        $('#photo_large').html('<img id="back_gallery" src="/site_media/img/backgallery.png" onclick="void(0)"/>' +
                                                '<div id="photo_large_inline_cont">' +
                                                    ...
                                                    '<span id="nominate_photo" class="awesome_abs red large">Nominate</span> ' +
    ...
    function video_gallery_view(){
        ...
        for (var i = 0; i < user_videos.length; i++){
            if (user_videos[i].id === photo_id){
                ...
                // $('.photo_thumbs').not($('#' + photo_id).parent()).filter(':hidden').css({
                //     'left': left_start,
                //     'top': '0px'
                // });
                // $('.photo_thumbs').attr('name', '');
                // $('.photo_thumbs').not($('#' + photo_id).parent()).show();
                // $('.photo_thumbs').not($('#' + photo_id).parent()).css({'opacity': 0.5, 'position': 'absolute'});
                // $('#' + photo_id).parent().css({'opacity': 1.0, 'position': 'absolute'});
                // $('#' + photo_id).parent().attr('name', 'selected');
                // 
                // $('.photo_thumbs:lt(' + before + ')').hide();
                // $('.photo_thumbs:gt(' + after + ')').hide();
                // 
                ...
                //     }
                //     else if (center_found === false){
                //         slide_center += slide_img_width + 40;
                //     }
                //     
                //     img_widths.push(slide_img_width + 10);
                // });
    ...
    function attach_main_handlers(){
        ...
        $('.friend').live('click', function(){
            ...
            window.location.hash = '/alpha_key=' + alpha_key;
        });
    }
    ...
    function update_nom_stream_vote_count(nom_id, vote_count){
        $('#nom_photo_' + nom_id + ' p').text(vote_count);
    }
    ...
    function vote_up_handler(){
        ...
        $.post('/vote/', {'method': 'up', 'nomination_id': nomination_id}, function(data){
            if (data){
                ...
                update_nom_stream_vote_count(nomination_id, data.vote_count);
            }
        });
    }
    ...
    function attach_profile_handlers(){
        ...
        // $('#profile_wrap  > ul').live('mouseover mouseout', function(event) {
        //     if (event.type == 'mouseover') {
        //         $('#drop_down_cont').show();
        //     } else {
        //         $('#drop_down_cont').hide();
        ...
        // $('.activate_stream').live('click', function(){
        //     var view_to_activate = $(this).attr('name');
        //     var active_view = $('#active_stream_view').attr('name');
        //     var active_text = $('#active_stream_view').text();
        //     
        //     if(view_to_activate == 'active_nominations'){
        //         $('#active_stream_view').html('Active Nominations<img id="stream_triangle" src="/site_media/img/downtriangle.png">').attr('name', view_to_activate);
        //         $('#active_stream_text_cont').show();
        //         window.location.href = '#/';
        //         // init_profile_view();
        //     }
        //     else if (view_to_activate == 'recent_winners'){
        //         $('#active_stream_view').html('Recent Winners<img id="stream_triangle" src="/site_media/img/downtriangle.png">').attr('name', view_to_activate);
        //         $('#active_stream_text_cont').hide();
        //         window.location.href = '#/recent-winners';
        //     }
        //     else if(view_to_activate == 'latest_photos'){
        //         $('#active_stream_view').html('Latest Photos<img id="stream_triangle" src="/site_media/img/downtriangle.png">').attr('name', view_to_activate);
        //         $('#active_stream_text_cont').hide();
        //         window.location.href = '#/latest-photos';
        //     }
        //     $('#drop_down_cont').hide();
        //     $('.activate_stream[name="' + view_to_activate + '"] h1').text(active_text).parent().attr('name', active_view);
        // });
        ...
        $('.next_photo').live('click', function(){
            $(this).parent().prev().children().filter('[name="selected"]').next().click();
        });
        
        $('.prev_photo').live('click', function(){
            $(this).parent().prev().children().filter('[name="selected"]').prev().click();
        });
        ...
        $('.add_new_comment').live('click', function(){
            ...
            if (!mobile){
                $(this).fadeOut();
                $(this).parent().parent().find('.new_comment_cont').slideDown(function(){
    ...
    function attach_nom_detail_handlers(){
        ...
        $('.next_photo').live('click', function(){
            $('#nom_cat_stream').children().filter('[name="selected"]').next().click();
        });
        
        $('.prev_photo').live('click', function(){
            $('#nom_cat_stream').children().filter('[name="selected"]').prev().click();
        });
    }
    ...
    function attach_user_profile_handlers(){
        ...
        $('#profile_nav_cont').live('click', function(){
            ...
            if (view_to_activate == 'albums'){
                $(this).attr('value', 'trophies');
                $('h2', this).text('Trophy Room');
                ...
                $(this).attr('value', 'albums');
                $('h2', this).text('Photo Albums');
    ...
    function attach_gallery_handlers(){
        if (mobile){
            ...
            $('#photo_large_inline_cont').live('click', load_next_picture);
        }
    ...
    function clear_event_handles(){    
        ...
        //Upgrade
        $('.friend_ref_cont').die('click');
        $('.kill_referral').die('click');
    ...
    function update_view(){     
        ...
        if (url_vars.user !== undefined && url_vars.album === undefined && url_vars.gallery === undefined){
            clear_canvas(url_vars);
            ...
            if (url_vars.album !== 'video_album'){
                ...
                if ($('#photo_list').children().length === 0){
                    ...
                    $('#photo_list').append('<div id="album_link_loader"><h2>Loading ' + user_name + '\'s Albums...</h2><img src="/site_media/img/loader_main.gif"/></div>');
