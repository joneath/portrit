$(document).ready(function(){
    ...
    function render_settings(){
        var setttings_html = '',
            ...
            allow_notifications_checked_class = 'unchecked',
            allow_portrit_album_checked = 'false',
            allow_portrit_album_checked_class = 'unchecked';
        
        if (allow_notifications){
            allow_notifications_checked = 'true';
            allow_notifications_checked_class = 'checked';
        }
        
        if (allow_portrit_album){
            allow_portrit_album_checked = 'true',
            allow_portrit_album_checked_class = 'checked';
        }
        ...
        setttings_html = '<div id="settings_cont">' +
                            ...
                            '<div id="allow_notifications_cont">' +
                                ...
                                '<div id="allow_notifications" value="fb_auto_post" class="' + allow_notifications_checked_class + '" checked="' + allow_notifications_checked + '"></div>' +
                                '<div class="clear"></div>' +
                            '</div>' +
                            '<div id="allow_portrit_album_cont">' +
                                ...
                                '<div id="allow_portrit_album" value="portrit_album" class="' + allow_portrit_album_checked_class + '" checked="' + allow_portrit_album_checked + '"></div>' +
                                '<div class="clear"></div>' +
                            '</div>' +
        ...
        var initial_tut_html =  '<div id="initial_tut_cont">' +
                                    '<div id="initial_tut_wrap">' +
                                        '<div id="initial_tut_top_wrap">' +
                                            ...
                                            '<div id="allow_notifications_cont">' +
                                                '<label for="allow_notifications">Allow Portrit to notify your Facebook account: </label>' +
                                                '<div id="allow_notifications" value="fb_auto_post" class="checked" checked="true"></div>' +
                                                '<div class="clear"></div>' +
                                            '</div>' +
                                            '<div id="allow_portrit_album_cont">' +
                                                '<label for="allow_portrit_album">Allow Portrit to post winning trophies back to Facebook: </label>' +
                                                '<div id="allow_portrit_album" value="portrit_album" class="checked" checked="true"></div>' +
                                                '<div class="clear"></div>' +
                                            '</div>' +
                                        '</div>' +
            ...
            if ($('#wall_cont #profile_wrap').length == 0){
                ...
                var wall_html = '<div id="profile_wrap">' +
                                    ...
                                    '<div id="active_stream_text_cont">' +
                                        '<a id="top_noms" class="sick large">Top</a>' +
                                        '<a id="recent_noms" class="sick large">Most Recent</a>' +
                                    '</div>' +
                                    '<div class="clear"></div>' +
    ...
    function render_albums(offset){
        ...
        if (albums.length > 1){
            ...
            if (i >= album_top){
                $('#album_cont').append('<a id="load_more_fb_albums" class="sick_abs large red">Load More</a>');
            }
        }
        else{
    ...
    function init_latest_photos(){
        ...
        append_load($('#context_overlay_cont > div'), 'light');
        if (!photo_feed || photo_feed.length == 0){
            ...
            $.getJSON('https://api.facebook.com/method/stream.get?access_token=' + fb_session.access_token + '&limit=50&filter_key=' + photo_filter + '&format=json&callback=?', function(data){
                ...
                clearTimeout(photo_feed_timeout);
                photo_feed_timeout = setTimeout(function(){
                    photo_feed = null;
                }, 1000 * 60 * 5);
            });
        }
    ...
    function init_recent_view(){
        ...
        $.getJSON('/init_recent_stream/', function(data){
            ...
            if (data.recent.length > 0){
                ...
                //     photo_feed = data.posts;
                //     // render_latest_photos(photo_feed);
                //     render_latest_photos_top_stream(photo_feed);
                //     clearTimeout(photo_feed_timeout);
                //     photo_feed_timeout = setTimeout(function(){
                //         photo_feed = null;
                //     }, 1000 * 60 * 5);
    ...
    function attach_upgrade_handlers(){
        $('.friend_ref_cont').live('click', function(){
            ...
            if ($('#refer_button').length === 0){
                $('#refferal_cont').append('<h3>Tell your friends how much you love Portrit!</h3><textarea id="refer_post" class="comment_body"></textarea><a id="refer_button" class="sick">Refer 1 friend</a>');
    ...
    function attach_profile_handlers(){
        ...
        $('#show_upload').live('click', function(){
            ...
            //     $('#drag_container').show();
            // }
            var photo_upload_html = '<a id="file_uploader">' +
                                        'Upload photo' +
                                    '</a>';
    ...
    function attach_album_handlers(){
        ...
        $('#load_more_fb_albums').live('click', function(){
            ...
            user_album_height_interval = setInterval(function(){
                if (user_album_height_interval_count < 20){
                    var album_current_height = $('#album_cont').height();
                    var current_min_height = $('#user_profile_cont').css('height');

                    if (current_min_height != album_current_height || current_min_height < album_current_height){
                        $('#user_profile_cont').css('height', album_current_height);
                        user_album_height_interval_count += 1;
                    }
                    else{
                        clearInterval(user_album_height_interval);
    ...
    function append_wall_html(view_to_activate){
        ...
        // invite_friends = '<h3 class="sick large invite_friends">Invite Friends</h3>';
        // 
        // if (mobile && !tablet){
        //     invite_friends = '';
        // }
